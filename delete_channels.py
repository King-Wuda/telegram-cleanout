...
def build_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Leave/delete Telegram channels and supergroups."
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="Also include supergroups/megagroups (default: broadcast channels only).",
    )
    parser.add_argument(
        "--exclude",
        nargs="+",
    ...
    parser.add_argument(
        ...
        default=[],
        dest="exclude_ids",
        help="Exclude by Telegram channel ID.",
    )
...
async def collect_channels(
    ...
    async for dialog in client.iter_dialogs():
        ...
        if is_supergroup and not include_supergroups:
            continue
...
def print_channel_list(channels: list) -> None:
...
async def delete_channels(client: TelegramClient, channels: list) -> None:
    total = len(channels)
    deleted = 0
    failed = 0
    ...
    for i, dialog in enumerate(channels, 1):
        ...
        try:
            ...
            print(f"[{i}/{total}] Left/deleted: {name}")
            deleted += 1
            ...
            try:
                await client.delete_dialog(dialog.entity)
                print(f"[{i}/{total}] Left/deleted (after wait): {name}")
                deleted += 1
    ...
    print(f"\nSummary: {deleted} left/deleted, {failed} failed, {skipped} skipped.")
...
async def main() -> None:
    ...
    async with TelegramClient("telegram_cleanup", api_id, api_hash) as client:
        ...
        channels = await collect_channels(
            client, args.include_supergroups, args.exclude, args.exclude_ids
        )

        if not args.execute:
            print("DRY RUN — no channels will be left/deleted. Pass --execute to proceed.")
        ...
        answer = input(f"Type DELETE to confirm leaving/deleting {len(channels)} channels: ")
        if answer.strip() != "DELETE":
            print("Aborted.")
            return

        await delete_channels(client, channels)
